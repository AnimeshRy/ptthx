...
// ErrWithResponseData repsresents http error with body, status code and headers
...
func JSONDecoder(r io.Reader, v interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("pttx.JSONEncoder:%w", err)
	}
	return nil
}
...
func JSONEncoder(w io.Writer, v interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("ptthx:JSONEncoder:%w", err)
	}
...
func FormURLEncodedDecoder(r io.Reader, v interface{}) error {
	...
	_, err := io.Copy(&b, r)
	if err != nil {
		return fmt.Errorf("ptthx:FormURLEncodedDecoder:read:%w", err)
	}

	val, err := url.ParseQuery(b.String())
	if err != nil {
		return fmt.Errorf("ptthx:FormURLEncodedDecoder:parse:%w", err)
	}
...
func FormURLEncodedEncoder(w io.Writer, v interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("ptthx:FormURLEncodedEncoder:%w", err)
	}
...
func BytesDecoder(reader io.Reader, v interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("ptthx:BytesDecoder:%w", err)
	}
...
func BytesEncoder(w io.Writer, v interface{}) error {
	if _, err := w.Write(v.([]byte)); err != nil {
		return fmt.Errorf("ptthx:BytesEncoder:%w", err)
	}
